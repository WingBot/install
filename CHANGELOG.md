...
- 使用 Termius 官方 Linux `.deb` 下载入口安装 amd64 版本。
- 下载过程复用可见流式进度和本地/系统代理自动检测逻辑，安装阶段直接显示 apt 原生日志。
...
- VS Code apt 安装改为直接输出原生命令日志，便于查看依赖安装和错误信息。
- VS Code 下载补充系统代理和常见 Clash 本地端口自动检测。
